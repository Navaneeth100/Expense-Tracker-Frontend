...
export default function PaymentMethod() {
    ...
    const fetchCategories = async () => {
        try {
            const res = await axios.get(`${url}/api/payment-method/`, {
                headers: { Authorization: `Bearer ${token}` },
            });
            setPaymentMethod(res.data?.results || res.data);
        } catch (e) {
            console.error(e);
        } finally {
            setLoading(false);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (editingId) {
                // UPDATE
                await axios.put(
                    `${url}/api/payment-method/${editingId}/`,
                    form,
                    { headers: { Authorization: `Bearer ${token}` } }
                );
            } else {
    ...
    const handleDelete = async (id) => {
        ...
        try {
            await axios.delete(`${url}/api/payment-method/${id}/`, {
                headers: { Authorization: `Bearer ${token}` },
            });
            fetchCategories();
        } catch (e) {
                ...
                <>
                    {paymentmethod?.length ? (
                        ...
                        <div className="flex justify-center items-center min-h-[40vh]">
                            <p className="text-gray-600 text-lg">No Payment Method found</p>
                        </div>
                    )}
                </>
            )}
